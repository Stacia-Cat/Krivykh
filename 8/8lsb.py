...
with open("./settings.txt", "r") as f_settings:
    settings_data = [float(num) for num in f_settings.read().split("\n")]
...
charge_plot_line, = axes.plot(charge_data[0], charge_data[1], color = 'purple')
...
axes.text(x = (charge_time/2-0.8), y = volt_max/2, s = ("Charge time: " + str(round(charge_time, 2)) + " s"), color = 'purple', fontsize = 14)
